...
def json_load(s, top_level_key=None):
    ...
    # print(f"Raw input to json_load:\n{s}")
    # if s is None:
    #     return None
    # json_start = s.find("{")
    # json_end = s.rfind("}")
    ...
    try:
        ...
        # Extract top-level key if provided
        if top_level_key:
            if top_level_key not in parsed_json:
                raise KeyError(f"Key '{top_level_key}' not found in JSON.")
            return parsed_json[top_level_key]

        return parsed_json
    except json.JSONDecodeError as e:
        print(f"JSONDecodeError: {e}")
        print(f"Invalid JSON input: {s}")
        return None
        ...
        for ex in res_dict:
            ...
            if "example_id" in ex:
                ...
                if len(text_list) > 0:
                    ...
                    text = text_list[0]
                    print(f"Adding row for doc_id: {doc_id} with concept_id: {concept_id}")  # <-- ADD THIS
...
async def score_helper(concept, batch_i, concept_id, df, text_col, doc_id_col, model_name, batch_size, get_highlights, sess, threshold):
    ...
    print(f"Scoring concept_id: {concept_id} with name: {concept.name}")
    ...
    cur_summary = f'Summary for concept: {concept.name} ({concept_id}): {cur_summary}'
    print(f"Results for concept: {concept.name} ({concept_id})")
    # print(f'Curr summary: {cur_summary}')
    # print(f'Score df: {score_df}')
    return score_df, cur_summary
...
async def score_concepts(text_df, text_col, doc_id_col, concepts, model_name="gpt-3.5-turbo", batch_size=5, get_highlights=False, sess=None, threshold=1.0):
    ...
    results = await tqdm_asyncio.gather(*tasks, file=sys.stdout)
    score_dfs, summaries = zip(*results)
